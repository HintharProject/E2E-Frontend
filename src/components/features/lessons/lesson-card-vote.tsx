...
export function LessonCardVote({ lessonId, initialVoteCount, initialUserVote }: { lessonId: string; initialVoteCount: number; initialUserVote?: number | null }) {
  ...
  const handleVote = (e: React.MouseEvent) => {
    ...
    timerRef.current = setTimeout(() => {
      voteMutation.mutate({ lessonId, value: targetValue });
    }, 1000);
  };
